...
export const SCENARIOS: ScenarioPreset[] = [
  ...
  {
    id: 'max-recon-rules',
    title: 'Cobertura Máxima: Crawling Profundo & Regras CWE',
    subtitle: 'Varredura profunda mesclada em relatório único',
    category: 'Completo',
    command: 'python cwe_discover.py -u https://alvo-autorizado.com --i-have-authorization --max-depth 6 --max-urls 1500 --rules --format html -o relatorio_completo.html',
    explanation: 'Combina profundidade máxima de crawling com o motor declarativo de regras nativo, gerando um relatório HTML unificado com achados passivos e determinísticos.',
    tags: ['Deep Crawl', 'Rules Engine', 'HTML Master']
  },
  {
...
# full_scan.sh - Automação completa de Recon + Mapeamento + Regras + HackerOne
DOMAIN=$1
if [ -z "$DOMAIN" ]; then
...
echo "[1/4] Descobrindo subdomínios via Certificate Transparency e OSINT passivo..."
python cwe_discover.py -u "$DOMAIN" --subdomains --i-have-authorization -o subs.txt

echo "[2/4] Executando Probing HTTP e fingerprinting de tecnologias..."
python cwe_discover.py -u "https://$DOMAIN" --probe --i-have-authorization -o probe.json

echo "[3/4] Executando motor declarativo de regras e matchers CWE..."
python cwe_discover.py -u "https://$DOMAIN" --rules --i-have-authorization -o rules_results.json

echo "[4/4] Executando pipeline completo e gerando dossiê no formato HackerOne..."
