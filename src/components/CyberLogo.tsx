      ...
      {showText && (
        <div className="flex flex-col leading-none min-w-0">
          <div className="flex items-center gap-1.5 whitespace-nowrap">
            <span className={`font-mono font-bold tracking-tight text-slate-100 whitespace-nowrap ${text}`}>
              cwe<span className="text-emerald-400">-</span>discover
            </span>
            <span className="text-[9px] sm:text-[10px] font-mono px-1.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 shrink-0">
              v1.0
            </span>
          </div>
          {subtitle && (
            <span className={`font-mono text-cyan-400/90 tracking-wide mt-0.5 truncate max-w-[180px] sm:max-w-none ${sub}`}>
              {subtitle}
            </span>
          )}
        </div>
