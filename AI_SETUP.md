...
## ⚙️ 3. Estrutura do Arquivo `ai_config.json`

O arquivo `ai_config.json` centraliza as diretrizes de IA:

```json
{
  "project": "CWE-Cover 2.0",
  "ollama": {
    "enabled": false,
...
| :--- | :--- | :--- |
| `Failed to connect to localhost:11434` | Ollama não está rodando em segundo plano | Execute `ollama serve` em outro terminal |
| `model 'llama3' not found` | O modelo ainda não foi baixado | Execute `ollama pull llama3` |
| `ImportError: cannot import name 'Crew'` | Pacote `crewai` ausente | Execute `pip install crewai` |
| `GEMINI_API_KEY not set` | Variável de ambiente não declarada | Execute `export GEMINI_API_KEY="..."` |
